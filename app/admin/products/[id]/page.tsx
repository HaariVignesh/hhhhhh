...
export default function EditProductPage() {
  ...
  useEffect(() => {
    if (autoSlug) setValue("slug", slugify(watchedName));
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        if (res.ok) {
          ...
          setValue("images", [
            ...currentImages,
            { url, altText: watchedName, isPrimary: currentImages.length === 0 },
          ]);
        }
      }
    } finally {
  ...
  const removeImage = (index: number) => {
    ...
    if (imgs.length > 0 && !imgs.some((img) => img.isPrimary)) imgs[0].isPrimary = true;
  ...
  const setPrimary = (index: number) => {
    setValue(
      "images",
      watch("images").map((img, i) => ({ ...img, isPrimary: i === index }))
    );
  };
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        <Tabs defaultValue="basic" className="space-y-5">
          ...
          >
            ...
            <div className="border-t border-zinc-100 pt-5">
              <p className="text-sm font-medium text-zinc-700 mb-4">Product Flags</p>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <label className="text-sm text-zinc-700">{f.label}</label>
                      </div>
                    )}
                  />
                ))}
                  ...
                  <div key={i} className="relative group">
                    <div
                      className={`relative aspect-square rounded-lg overflow-hidden border-2 cursor-pointer transition-colors ${
                        img.isPrimary ? "border-zinc-900" : "border-zinc-200 hover:border-zinc-400"
                      }`}
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-zinc-900">Product Variants</h3>
                <p className="text-xs text-zinc-500 mt-0.5">Size, color and stock</p>
              </div>
              <Button
                type="button"
                variant="outline"
            ...
            <div className="space-y-3">
              {variantFields.map((field, index) => (
                ...
                >
                  <div>
                    <label className="text-[11px] text-zinc-500 mb-1 block">Size</label>
                    <Input {...register(`variants.${index}.size`)} placeholder="M" className="h-8 text-sm" />
                  </div>
                  <div>
                    <label className="text-[11px] text-zinc-500 mb-1 block">Color</label>
                    <Input {...register(`variants.${index}.color`)} placeholder="Ivory" className="h-8 text-sm" />
                  </div>
                  <div>
                    <label className="text-[11px] text-zinc-500 mb-1 block">Hex</label>
                    <Input type="color" {...register(`variants.${index}.colorHex`)} className="h-8 px-1 py-0.5 cursor-pointer" />
                  </div>
                  <div>
                    <label className="text-[11px] text-zinc-500 mb-1 block">SKU</label>
                    <Input {...register(`variants.${index}.sku`)} placeholder="SKU-001" className="h-8 text-sm" />
                  </div>
                  <div>
                    <label className="text-[11px] text-zinc-500 mb-1 block">Stock</label>
                    <Input type="number" {...register(`variants.${index}.stock`, { min: 0 })} placeholder="0" className="h-8 text-sm" />
                  </div>
                  <div className="flex items-end gap-2">
                    <div className="flex-1">
                      <label className="text-[11px] text-zinc-500 mb-1 block">Price ₹</label>
                      <Input type="number" step="0.01" {...register(`variants.${index}.price`)} placeholder="Override" className="h-8 text-sm" />
                    </div>
                    <button
                      type="button"
              ...
              {variantFields.length === 0 && (
                <p className="text-sm text-zinc-400 text-center py-8">
                  No variants added. Click &ldquo;Add Variant&rdquo; to add size/color options.
                </p>
              )}
            </div>
          </TabsContent>
          ...
          >
            <div>
              ...
              <Input {...register("metaTitle")} placeholder="SEO page title" className={inputClass} maxLength={60} />
              <p className="text-xs text-zinc-400 mt-1">Recommended: 50–60 characters</p>
            </div>
            <div>
              <label className={fieldClass}>Meta Description</label>
              <Textarea {...register("metaDescription")} placeholder="Brief description for search engines..." rows={3} maxLength={160} className="resize-none" />
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          <Button type="submit" disabled={submitting} className="bg-zinc-900 hover:bg-zinc-800 gap-2 min-w-32">
